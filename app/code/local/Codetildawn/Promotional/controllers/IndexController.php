...
class Codetildawn_Promotional_IndexController extends Mage_Core_Controller_Front_Action
{
    public function popupajaxAction()
    {
        ...
        try {
            ...
            if (isset($popup['promotional_id'])) {
				Mage::getSingleton('core/session')->unsPopupId();
				Mage::getSingleton('core/session')->setPopupId($popup['promotional_id']);
	...
	public function popupajaxcloseAction(){
		$popupId = Mage::getSingleton('core/session')->getPopupId();
		if($popupId){
			Mage::helper('promotional')->setViewedPopup($popupId);
		}
		return;
	}
}
