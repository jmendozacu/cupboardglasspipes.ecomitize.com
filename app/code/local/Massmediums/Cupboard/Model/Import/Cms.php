...
class Massmediums_Cupboard_Model_Import_Cms extends Mage_Core_Model_Abstract
{
	/**
    ...
    {
        parent::__construct();
		$this->_importPath = Mage::getBaseDir() . '/app/code/local/Massmediums/Cupboard/etc/import/';
    }
	...
	public function importCmsItems($modelString, $itemContainerNodeString, $overwrite = false)
    {
		try
		{
			...
			{
				throw new Exception(
					Mage::helper('cupboard')->__("Can't read data file: %s", $xmlPath)
					);
			}
			...
			if ($i)
			{
				Mage::getSingleton('adminhtml/session')->addSuccess(
					Mage::helper('cupboard')->__('Number of imported items: %s', $i)
				);
			}
			else
			{
				Mage::getSingleton('adminhtml/session')->addNotice(
					Mage::helper('cupboard')->__('No items were imported')
				);
			}
