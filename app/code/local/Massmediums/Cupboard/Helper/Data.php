...
class Massmediums_Cupboard_Helper_Data extends Mage_Core_Helper_Abstract
{
	/**
	 * Patterns
	 *
	...
	{
		//Create paths
		$this->_texPath = 'wysiwyg/massmediums/cupboard/_patterns/default/';
		$this->_bgImagesPath = 'wysiwyg/massmediums/cupboard/_backgrounds/';
	}
    ...
    {
		if ($storeId)
			return Mage::getStoreConfig('cupboard/' . $group, $storeId);
		else
			return Mage::getStoreConfig('cupboard/' . $group);
    }
    ...
    {
		if ($storeId)
			return Mage::getStoreConfig('cupboard_design', $storeId);
		else
			return Mage::getStoreConfig('cupboard_design');
    }
    ...
    {
        return Mage::getStoreConfig('cupboard/' . $optionString);
    }
    ...
    {
        return Mage::getStoreConfig('cupboard_design/' . $optionString, $storeCode);
    }
    ...
    {
        return Mage::getStoreConfig('cupboard_layout/' . $optionString, $storeCode);
    }
	...
	public function getAltImgHtml($product, $w, $h, $imgVersion='small_image')
	{
		...
		if ($gal = $product->getMediaGalleryImages())
		{
			...
			{
				return
				'<img class="alt-img" src="' . Mage::helper('massmediums/image')->getImg($product, $w, $h, $imgVersion, $altImg->getFile()) . '" alt="' . $product->getName() . '" />';
			}
		}
